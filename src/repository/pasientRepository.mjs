import {
  ...
  passientModel,
  predictionsModel,
} from "../model/defineDatabaseRelations.mjs";
...
const pasientRepository = {
  ...
  async createNewpassientWithOcr(ocr, idAccount) {
    ...
    const pasientData = {
      ...
      nama: parsedOcr.nama,
      nik: parsedOcr.nik,
      jenisKelamin: parsedOcr.jenis_kelamin.toUpperCase().startsWith("L")
        ? "L"
        : "P",
      tanggalLahir: new Date(
        parsedOcr.tanggal_lahir.split("-").reverse().join("-")
      ),
  ...
  async fetchPassientById(pasientId) {
  ...
  async fetchPasientByAccountId(accountId) {
